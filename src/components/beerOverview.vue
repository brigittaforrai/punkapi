    ...
    <v-card style="padding-bottom: 50px;">
      ...
      <v-layout column>
        <v-flex v-bind="{ [`xs${size}`]: true }">
          <v-img :lazy-src="placeholder" :src="beer.image_url || placeholder" height="350px" contain></v-img>
        </v-flex>
        ...
        <v-flex v-bind="{ [`xs${size}`]: true }">
          <v-card-title primary-title>
            <div>
              ...
              <div v-if="details" class="subheading font-weight-bold">
                ...
                <ul v-for="food in beer.food_pairing" :key="food">
          ...
          <section v-if="details" style="padding: 16px;">
            ...
            <ul :key="key" v-for="(value, key) in beer.ingredients">
              <li>
                ...
                <span v-if="value[k].name && value[k].name.length"
                      ...
                      v-for="(v, k) in value" :key="k">{{value[k].name}}</span>
              </li>
            </ul>
          </section>
